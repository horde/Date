...
class Horde_Date_Repeater_Week extends Horde_Date_Repeater
{
    ...
    public function next($pointer = 'future')
    {
        ...
        return new Horde_Date_Span($this->currentWeekStart, $this->currentWeekStart->add(['day' => 7]));
    }
    ...
    public function this($pointer = 'future')
    {
        ...
        switch ($pointer) {
            case 'future':
                $thisWeekStart = new Horde_Date(['year' => $this->now->year, 'month' => $this->now->month, 'day' => $this->now->day, 'hour' => $this->now->hour + 1]);
                $sundayRepeater = new Horde_Date_Repeater_DayName('sunday');
                $sundayRepeater->now = $this->now;
            ...
            case 'past':
                $thisWeekEnd = new Horde_Date(['year' => $this->now->year, 'month' => $this->now->month, 'day' => $this->now->day, 'hour' => $this->now->hour]);
                $sundayRepeater = new Horde_Date_Repeater_DayName('sunday');
                $sundayRepeater->now = $this->now;
                $lastSundaySpan = $sundayRepeater->next('past');
    ...
    {
        $direction = ($pointer == 'future') ? 1 : -1;
        return $span->add(['day' => $direction * $amount * 7]);
    }
