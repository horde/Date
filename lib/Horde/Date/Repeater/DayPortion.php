...
class Horde_Date_Repeater_DayPortion extends Horde_Date_Repeater
{
     ...
     */
    public static $morning = [21600, 43200];

    /**
     * 1pm-5pm (13 * 60 * 60, 17 * 60 * 60)
     */
    public static $afternoon = [46800, 61200];

    /**
     * 5pm-8pm (17 * 60 * 60, 20 * 60 * 60)
     */
    public static $evening = [61200, 72000];

    /**
     * 8pm-12pm (20 * 60 * 60, 24 * 60 * 60)
     */
    public static $night = [72000, 86400];
    ...
    public function __construct($type)
    {
        ...
        if (is_int($type)) {
            $this->range = [($type * 3600), (($type + 12) * 3600)];
        } else {
            $lookup = [
                'am' => [0, (12 * 3600 - 1)],
    ...
    public function next($pointer = 'future')
    {
        ...
        if (!$this->currentSpan) {
            ...
            if ($nowSeconds < $this->range[0]) {
                switch ($pointer) {
                    case 'future':
                        $rangeStart = new Horde_Date(['year' => $this->now->year, 'month' => $this->now->month, 'day' => $this->now->day, 'sec' => $this->range[0]]);
                        break;

                    case 'past':
                        $rangeStart = new Horde_Date(['year' => $this->now->year, 'month' => $this->now->month, 'day' => $this->now->day - 1, 'sec' => $this->range[0]]);
                        break;
                }
            ...
            switch ($pointer) {
                case 'future':
                    $this->currentSpan = $this->currentSpan->add(['day' => 1]);
                    break;
    ...
    public function offset($span, $amount, $pointer)
    {
        ...
        return $portionSpan->add(['day' => $direction * ($amount - 1)]);
    }
